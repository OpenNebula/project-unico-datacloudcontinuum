...
class OpenNebulaPowerCollector
    ...
    HOST_POWER_METRICS = {
        'host_power_consumption_uW' => {
            ...
            :value => lambda {|v|
                Integer(@scaph_data[Integer(v['ID'])]\
    ...
    def query_vm_agent(vm_id, query, host = 'localhost')
        cmd = "virsh -c 'qemu+ssh://#{host}/system' \
            qemu-agent-command one-#{vm_id} '#{query.to_json}'"
        out = `#{cmd}`
        JSON.parse(out)
    end
    ...
    def get_proc_consumption(vm_id, host = localhost, full_power = 0)
        ...
        pid_cmd = issue_command(vm_id, host, cmd,
                                "-s #{interval} -m #{vm_id} -p #{full_power}")
    rescue StandardError
        ...
        procs_info.each_line do |l|
            vm_id, vm_proc, vm_cmd, power = l.split("\|")
    ...
    def initialize(registry, client, namespace)
        ...
        [HOST_POWER_METRICS, VM_POWER_METRICS, VMPROC_POWER_METRICS].each do |m|
            m.each do |name, conf|
    ...
    def collect
        ...
        hosts.each do |host|
            ...
            HOST_POWER_METRICS.each do |name, conf|
